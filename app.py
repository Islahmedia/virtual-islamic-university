...
# Support both localhost and 127.0.0.1 on multiple ports for VS Code compatibility
CORS(app, 
...
class AdmissionApplication(db.Model):
    ...
    def __init__(self, **kwargs):
        ...
        if not self.application_number:
            self.application_number = f"VIU-{datetime.now().year}-{self.id or 0:06d}"
...
def submit_contact():
    ...
    try:
        ...
        admin_subject = f"نیا رابطہ پیغام - {data['subject']}"
        admin_body = f"""
...
وقت: {contact.submission_date.strftime('%Y-%m-%d %H:%M:%S')}
Submission ID: {contact.id}

Virtual Islamic University
